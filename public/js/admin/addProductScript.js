document.addEventListener("DOMContentLoaded", function () {
   ...
   productNameContainer.addEventListener('blur',()=>{
    console.log('Call Inside productNameContainer')
    const value = productNameContainer.value.trim();
    if(!value){
      showError('name','Product name is required');
    }else if(value.length<3){
      showError('name','Product name should be at least 3 characters');
    }
   })

   categoryContainer.addEventListener("blur", () => {
    if (!categoryContainer.value.trim()) {
      showError("category", "Category is required");
    }
  });

  subCategoryContainer.addEventListener("change", () => {
    const value = subCategoryContainer.value.trim();
    const subCategories = ['Beginner','Intermediate','Advanced'];
    removeError('subCategory');
    if (!value) {
      showError("subCategory", "Subcategory is required");
    }else if(!subCategories.includes(value)){
      showError('subCategory','SubCategory Should Beginner,Intermediate or advanced')
    }
  });

  brandContainer.addEventListener('blur',()=>{
    const value = brandContainer.value.trim();
    if(!value){
      showError('brand','brand name is required');
    }else if(value.length<3){
      showError('brand','brand name should be at least 3 characters');
    }
   })

   description1Container.addEventListener("blur", () => {
    const value = description1Container.value.trim();
    if (!value) {
      showError("description1", "Primary description is required");
    }else if (value.length < 10) {
      showError('description1','"Description 1 must be at least 10 characters"')
    } else if (value.length > 500) {
      showError('description1',"Description 1 must not exceed 500 characters") 
    }
  });

   description2Container.addEventListener("blur", () => {
    const value = description2Container.value.trim();
    if (!value) {
      showError("description1", "Primary description is required");
    }else if (value.length < 10) {
      showError('description1','"Description 1 must be at least 10 characters"')
  ...
  driverContainer.addEventListener('blur',()=>{
    const value = driverContainer.value.trim();
    if(!value){
      showError('driver','driver name is required');
    }else if(value.length<3){
      showError('driver','driver name should be at least 3 characters');
    }
   })


   driverConfigurationContainer.addEventListener('blur',()=>{
    const value = driverConfigurationContainer.value.trim();
    if(!value){
      showError('driverConfiguration','driverConfiguration is required');
    }else if(value.length<3){
      showError('driverConfiguration','driverConfiguration should be at least 3 characters');
    }
   })


   impedanceContainer.addEventListener('blur',()=>{
    const value = impedanceContainer.value.trim();
    if(!value){
      showError('impedence','Impedence name is required');
    }else if(isNaN(Number(value))){
      showError('impedence','Impedence should be number');
    }
   })

   soundSignatureContainer.addEventListener('blur',()=>{
    const value = soundSignatureContainer.value.trim();
    if(!value){
      showError('soundSignature','Sound Signature name is required');
    }
   })

   microphoneContainer.addEventListener('blur',()=>{
    const value = microphoneContainer .value.trim();
    if(!value){
      showError('microphone','microphone is required');
    }
   })
   productNameContainer.addEventListener('focus',()=>{
    removeError('name')
   })

  //  [productNameContainer,brandContainer,categoryContainer,subCategoryContainer,description1Container,description2Container,variantsContainer,driverContainer,driverConfigurationContainer,impedanceContainer,soundSignatureContainer,microphoneContainer].forEach((element)=>{
  //   if(element){
  ...
  //  })
   ...
   function showError(inputId,message){
    console.log('Call inside show error',inputId);
